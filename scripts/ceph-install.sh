...
echo "2. Installing ceph components -- skipped"
#RELEASE=hammer
#ceph-deploy --username cephadm install --release ${RELEASE} demo-admin
#
#for node in `cat /opt/nodes/masters`
#do
#    ceph-deploy --username cephadm install --release ${RELEASE} $node
#done
#
...
echo "9. Add in more monitors in cluster if available"
for mon in `sudo cat /opt/nodes/masters`
do
    if [ "$mon" != "demo-master-0" ]; then
        ceph-deploy --username cephadm mon create $mon
    fi
done
